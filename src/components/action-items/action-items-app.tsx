...
import { Bot, ChevronRight, ListFilter, Loader2, LogOut, Plus, Search, Sparkles } from "lucide-react";
...
type SessionResponse = {
  ...
  user: { id: string; portalUserId: string; name: string | null; handle: string | null; avatarUrl: string | null } | null;
  portalUrl: string;
};
type ItemPage = { items: ActionItem[]; page: { hasMore: boolean; nextCursor: string | null } };
type UserPage = { users: UserSummary[]; page: { hasMore: boolean; nextCursor: string | null } };
...
export function ActionItemsApp() {
  ...
  if (session.isLoading) return <Centered><Loader2 className="h-7 w-7 animate-spin text-primary" /></Centered>;
  if (!session.data?.authenticated) {
    return (
      <Centered>
        <div className="max-w-md rounded-lg border bg-card p-8 text-center shadow-2xl">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary"><ListFilter /></div>
          <h1 className="font-heading text-2xl font-semibold">Action Items</h1>
          <p className="mt-2 text-sm text-muted-foreground">Launch this module from RaidGuild Portal to establish your member session.</p>
          <Button className="mt-6" onClick={() => { window.location.href = session.data?.portalUrl || "https://portal.raidguild.org/modules"; }}>Return to Portal</Button>
        </div>
      </Centered>
    );
  }
    ...
    <main className="flex h-dvh min-h-0 flex-col">
      <header className="flex shrink-0 flex-wrap items-center gap-3 border-b bg-background/85 px-4 py-3 backdrop-blur md:px-6">
        <div className="mr-auto flex min-w-48 items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-md bg-primary text-primary-foreground"><ListFilter className="h-5 w-5" /></div>
          <div>
            <h1 className="font-heading text-lg font-semibold leading-tight">Action Items</h1>
            <p className="text-xs text-muted-foreground">P1 first · shared work queue</p>
          </div>
        </div>
        ...
        <div className="ml-1 flex items-center gap-2 border-l pl-3">
          <Avatar><AvatarImage src={member.avatarUrl ?? undefined} alt="" /><AvatarFallback>{initials(member.name || member.handle || "RG")}</AvatarFallback></Avatar>
          <button
            className="rounded p-2 text-muted-foreground hover:bg-muted hover:text-foreground"
            aria-label="Log out"
            onClick={async () => { await apiFetch("/api/session", { method: "DELETE" }); window.location.reload(); }}
      ...
      <section className="shrink-0 border-b bg-card/45 px-4 py-3 md:px-6">
        <div className="flex items-center gap-3">
          <div className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-full border border-primary/20 bg-primary/10 text-primary sm:flex"><Sparkles className="h-4 w-4" /></div>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium">Welcome back, {member.name || member.handle || "guild member"}.</p>
            <p className="truncate text-xs text-muted-foreground">Ask Prism to connect the next useful item with context and people.</p>
          </div>
          <Button variant="outline" size="sm" onClick={() => assistant.mutate()} disabled={assistant.isPending}>
            {assistant.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}
            What do I do?
          </Button>
        </div>
      ...
      <div
        ...
        onScroll={(event) => {
          ...
          if (node.scrollHeight - node.scrollTop - node.clientHeight < 500 && itemsQuery.hasNextPage && !itemsQuery.isFetchingNextPage) itemsQuery.fetchNextPage();
        }}
      >
        {itemsQuery.isLoading ? <Centered><Loader2 className="h-6 w-6 animate-spin text-primary" /></Centered>
          : itemsQuery.error ? <Centered><p className="text-sm text-destructive">{itemsQuery.error.message}</p></Centered>
          : !items.length ? <Centered><div className="text-center"><p className="font-medium">No action items found</p><p className="mt-1 text-sm text-muted-foreground">Try another filter or create the first one.</p></div></Centered>
          : (
        ...
        {itemsQuery.isFetchingNextPage && <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div>}
      </div>

      <ActionItemDialog open={dialogOpen} onOpenChange={setDialogOpen} itemId={selectedItemId} users={users.data?.users ?? []} />

      <Dialog open={assistantOpen} onOpenChange={setAssistantOpen}>
        <DialogContent className="max-w-xl p-0">
          <DialogHeader className="border-b px-6 py-5 pr-14">
            <DialogTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary" />Prism suggests</DialogTitle>
            <DialogDescription>Contextual guidance based on a small selection of actionable items.</DialogDescription>
          </DialogHeader>
          <div className="max-h-[65vh] overflow-y-auto whitespace-pre-wrap px-6 py-5 text-sm leading-7">{assistant.data?.guidance}</div>
        </DialogContent>
      </Dialog>
    </main>
  );
